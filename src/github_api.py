...
# PyGithub imports are needed again
    ...
    def _get_signature_source(self, node: ast.FunctionDef):
         try:
             ...
             if def_line_index != -1:
                 ...
                 start_index = 0 # Include decorators
        ...
        if line.startswith("+++ b/"):
            ...
            if match:
                current_line_in_new_file = int(match.group(1))
                 ...
                 # Reset if header format is unexpected
                 current_filename = None
             ...
             pass # Deleted line, doesn't advance line number in new file
        elif not line.startswith("@@") and current_filename:
             current_line_in_new_file += 1 # Context line
...
def get_pr_number_from_event() -> Optional[int]:
    ...
    try:
        ...
        logger.error(f"Error reading event payload: {e}", exc_info=True)
        return None

def get_pull_request(pr_number: int) -> Optional[PullRequest]:
    ...
    try:
        ...
        logger.error(f"Unexpected error getting PR #{pr_number}: {e}", exc_info=True)
        return None
...
def get_issue(issue_number: int) -> Optional[Issue]:
    ...
    try:
        ...
        logger.error(f"Unexpected error getting Issue #{issue_number}: {e}", exc_info=True)
        return None

# --- Core API Functions (Original PyGithub / Requests Based) ---

def get_pr_diff(pr: PullRequest) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred fetching diff for PR #{pr.number}: {e}", exc_info=True)
        return None


def find_linked_issue_number(pr: PullRequest) -> Optional[int]:
    ...
    try:
        ...
        for event in timeline:
            if event.event == 'cross-referenced' and event.source and event.source.issue:
                 ...
                 # Ensure it's from the same repo and not the PR itself
        ...
        logger.error(f"GitHub API error fetching timeline events for PR #{pr.number}: {e_outer.status} {e_outer.data}. Falling back to regex check.")
    except Exception as e_outer:
        logger.error(f"Unexpected error fetching timeline events for PR #{pr.number}: {e_outer}. Falling back to regex check.", exc_info=True)

    # --- Fallback: Regex check on PR body ---
    try:
        pr_body = pr.body
        if not pr_body:
            logger.warning(f"PR #{pr.number} body is empty. Cannot find linked issue via body text.")
            return None
        ...
        return None
         ...
         logger.error(f"An unexpected error occurred during regex fallback for PR #{pr.number}: {e_fallback}", exc_info=True)
         return None


def get_issue_body(issue: Issue) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred getting body for issue #{issue.number}: {e}", exc_info=True)
        return None
...
def post_pr_comment(pr_or_issue_number: int, comment_body: str) -> bool:
    ...
    if not comment_body:
        ...
        return False
    try:
        target_issue = repo.get_issue(pr_or_issue_number) # Get issue object first
        ...
        logger.error(f"An unexpected error occurred posting comment to #{pr_or_issue_number}: {e}", exc_info=True)
        return False


def get_contextual_code(pr: PullRequest, diff_text: str) -> str:
    """
    ...
    def get_parsed_file(filename: str) -> Optional[AstParser]:
        ...
        try:
            ...
            # Use PyGithub repo object to get contents
            ...
            return parser
             ...
             logger.error(f"File not found via PyGithub: {filename} at ref {pr_head_sha}.")
             return None
        except GithubException as e:
